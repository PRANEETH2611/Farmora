...
export default function CreatorDashboard() {
  ...
  const canViewSensitive = isAuthenticated && user?.role === "creator";
      ...
      <div className="space-y-2 relative z-10">
        ...
        {!canViewSensitive && (
          <p className="text-sm text-muted-foreground">
            You are viewing as <span className="font-semibold">{isAuthenticated ? "User" : "Guest"}</span>. Only leaderboard is visible.
          </p>
        )}
      </div>
      ...
      {canViewSensitive && (
        <>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <LeaderboardCard topCreators={topCreators} fairnessScore={fairnessScore} />
          </div>
        </>
      )}
...
function LeaderboardCard({ topCreators, fairnessScore }: { topCreators: Creator[]; fairnessScore: number }) {
  return (
