...
export default function TutorialCard({ tutorial, quantumSelected }: TutorialCardProps) {
  return (
    <Link href={`/tutorials/${tutorial.id}`}>
      <a className={`group block relative p-3 rounded-[1.5rem] transition-all duration-300 ${quantumSelected ? 'bg-primary/5 shadow-[0_0_20px_rgba(0,119,51,0.08)] border border-primary/20' : 'hover:bg-muted/50'}`} data-testid={`card-tutorial-${tutorial.id}`}>
        {quantumSelected && (
        ...
        <div className="space-y-2">
          <h3 className="font-bold text-xl leading-tight group-hover:text-primary transition-colors" data-testid={`text-title-${tutorial.id}`}>
            {tutorial.title}
          </h3>
          ...
          <div className="flex items-center justify-between text-[11px] text-muted-foreground pt-1">
            ...
            <div className="flex items-center gap-3">
              <span className="flex items-center gap-1"><Eye className="w-3 h-3" /> {tutorial.views.toLocaleString()}</span>
